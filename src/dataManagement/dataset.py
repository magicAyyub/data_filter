...
class DataSet:
    ...
    def save_csv(self, file_path:str) -> None:
        ...
        if len(self.data_list) > 0:
            with open(file_path, 'w', newline='') as file:
                fieldnames = self._all_to_dict()[0].keys()
                writer = csv.DictWriter(file, fieldnames=fieldnames)
                writer.writeheader()
                writer.writerows(self._all_to_dict())
        else:
    ...
    def save_json(self, file_path:str) -> None:
        ...
        if len(self.data_list) > 0:     
            with open(file_path, 'w') as file:
                json.dump(self._all_to_dict(), file, indent=2)     
        else:
            raise EmptyDataListError()
    ...
    def load_xml(self, file_path:str) -> None:
        ...
        try:
            ...
            root = tree.getroot()
            data =  [{child.tag.split('}')[-1]: child.text for child in item} for item in root]
            for dict_data in data:
                self.add_data(dict_data)
        except FileNotFoundError:
            raise FileNotFoundError(f"Aucun fichier ne correspond à \"{file_path}\"")
    ...
    def save_yaml(self, file_path:str) -> None:
        ...
        if len(self.data_list) > 0:      
            with open(file_path, 'w') as file:
                yaml.dump(self._all_to_dict(), file)      
        else:
            raise EmptyDataListError()
